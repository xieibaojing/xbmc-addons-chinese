...
def GetPPTVVideoURL_Flash(url, quality):
    ...
    #http://web-play.pptv.com/webplay3-0-300164.xml&ft=0&zone=8&vvid=4cd09c11-6785-2c65-4600-ea3533426a13&username=&param=type%3Dweb%26userType%3D0%26o%3D0&r=1356844054346
...
def GetPPTVVideoURL_ppsap(url, quality):
    ...
    if len(vid) <= 0:
        return []
    # get video server and file name
    #http://web-play.pptv.com/webplay3-0-300145.xml&ft=0&zone=8&vvid=e28707ae-960e-5821-9de1-eff4291441c4&username=&param=type%3Dweb%26userType%3D0%26o%3D0&r=1356940455364#
    #http://web-play.pptv.com/webplay3-0-300145.xml&ft=0&version=2&type=web.fpp
    
    data = unicode(GetHttpData(PPTV_WEBPLAY_XML + 'webplay3-0-' + vid + '.xml&ft=0&version=2&type=web.fpp'), 'utf-8', 'ignore')
    shes = parseDOM(data, 'sh')
    bhost = parseDOM(data, 'bh')
    ts = parseDOM(data, 'channel', ret = 'ts')
    rids = parseDOM(data, 'item', ret = 'rid')
    fts = parseDOM(data, 'item', ret = 'ft')
    brate = parseDOM(data, 'item', ret = 'bitrate')
    #print "decode: ", shes, bhost, ts, rids, fts, brate
    
    if min(len(shes), len(rids), len(fts)) <= 0:
       return []
    # get quality index
    try:
        ind = fts.index(str(quality))
...
def playVideo(name, url, thumb):
    ...
    if len(url) > 0:
        ...
        if len(ppurls) == 0 and ppsap_en == 'true':
