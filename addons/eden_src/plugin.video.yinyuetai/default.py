...
# 音悦台MV
######################################################
# Version 1.4.2 2012-06-18 (cmeng)
# a. img url link given as full path
#    => update def get_Thumb(icon):
######################################################
...
def ListRecommendMV(url,name,mode):
    link=get_content(url)
    if link == None: return
    match0=re.compile('<div class="page-nav" id="pageNav">(.+?)</div>').findall(link)   
...
ctl = {
            ...
            41   : ('get_artistpage(url,ctl[mode][1],mode,257)','内地男歌手','/fanAll?area=ML&property=Boy'),
            42   : ('get_artistpage(url,ctl[mode][1],mode,257)','内地女歌手','/fanAll?area=ML&property=Girl'),
            43   : ('get_artistpage(url,ctl[mode][1],mode,257)','内地乐队/组合','/fanAll?area=ML&property=Combo'),
            44   : ('get_artistpage(url,ctl[mode][1],mode,257)','港台男歌手','/fanAll?area=HT&property=Boy'),
            45   : ('get_artistpage(url,ctl[mode][1],mode,257)','港台女歌手','/fanAll?area=HT&property=Girl'),
            46   : ('get_artistpage(url,ctl[mode][1],mode,257)','港台乐队/组合','/fanAll?area=HT&property=Combo'),
            ...
            62   : ('ListRecommendMV(url,name,mode)','内地推荐MV','/lookVideo-area/ML'),
            63   : ('ListRecommendMV(url,name,mode)','港台推荐MV','/lookVideo-area/HT'),
            ...
            81   : ('ShowFocusMV(url,name,mode)','内地周榜','/index-ml'),
            82   : ('ShowFocusMV(url,name,mode)','港台周榜','/index-ht'),
