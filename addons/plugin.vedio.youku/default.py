...
def Video5(name,list,type,kind,sort,page):
		...
		xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li)
       		match=re.compile('<ulclass="video"><liclass="videoImg"><ahref="http://v.youku.com/v_show/id_(.+?).html"charset="(.+?)<imgsrc="(.+?)"alt="(.+?)"').findall(link)
                if len(match)>0:
        		for i in range(0,len(match)):
                ...
                match=re.compile('<divclass="LPageBar">(.+?)</div>').findall(link)
                if len(match)>0:
		...
		u=sys.argv[0]+"?mode=40&name="+urllib.quote_plus(name)
		xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li)
       		match=re.compile('<divclass="videoImg"><ahref="/playlist_show/id_(.+?).html"charset="(.+?)<imgsrc="(.+?)"alt="(.+?)"').findall(link)
                if len(match)>0:
        		for i in range(0,len(match)):
				li=xbmcgui.ListItem(str(i+1)+"."+match[i][3],match[i][2],match[i][2])
				u=sys.argv[0]+"?mode=8&name="+urllib.quote_plus(name+">"+match[i][3])+"&url="+urllib.quote_plus(match[i][0])+"&upage="+urllib.quote_plus("1")
				xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li,True)
...
def Video6(name,url,upage):
        url="http://www.youku.com/playlist_show/id_"+url+"_ascending_1_mode_pic_page_"+upage+".html"
        req = urllib2.Request(url)
       	req.add_header("User-Agent", "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3")
        response = urllib2.urlopen(req)
        link=response.read()
	...
	u=sys.argv[0]+"?mode=40&name="+urllib.quote_plus(name)
	xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li)
       	match=re.compile('<liclass="vImg"><imgsrc="(.+?)"alt="(.+?)"(.+?)id="PlayListFlag_(.+?)"(.+?)<spanclass="time">(.+?)</span>(.+?)<spanclass="num">(.+?)</span>').findall(link)
        if len(match)>0:
        	for i in range(0,len(match)):
			li=xbmcgui.ListItem(str(i+1)+"."+match[i][1]+"【"+match[i][5]+"，"+match[i][7].replace(',','')+"】",match[i][0],match[i][0])
			u=sys.argv[0]+"?mode=18&name="+urllib.quote_plus(match[i][1])+"&url="+urllib.quote_plus(match[i][3])+"&director="+urllib.quote_plus("")+"&studio="+urllib.quote_plus("")+"&plot="+urllib.quote_plus("")
			xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li)
       	match=re.compile('<divclass="pageBar">(.+?)</div>').findall(link)
        if len(match)>0:
   		match0=re.compile('<ahref="http://www.youku.com/playlist_show/id_"+url+"_ascending_1_mode_pic_page_(.+?).html"title=\'(.+?)\'>').findall(match[0])
        	for i in range(0,len(match0)):
		        li=xbmcgui.ListItem(".."+match0[i][1])
		       	u=sys.argv[0]+"?mode=8&name="+urllib.quote_plus(name)+"&url="+urllib.quote_plus(url)+"&upage="+urllib.quote_plus(match0[i][0])
		       	xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li,True)
...
def Movie(name,list):
        ...
        link=response.read()
       	response.close()
        #地区参数
        match=re.compile('href="/v_olist/c_'+list[1:]+'_a_(.+?)_s__g__r__d_1_fv_0_fl__o_7.html">(.+?)</a>').findall(link)
        for i in range(0,len(match)):
                strtype=strtype+"a_"+match[i][0]+","+match[i][1]+";"
        strtype=strtype+"|"
        #类型参数
        match=re.compile('href="/v_olist/c_'+list[1:]+'_a__s__g_(.+?)_r__d_1_fv_0_fl__o_7.html">(.+?)</a>').findall(link)
        for i in range(0,len(match)):
                strtype=strtype+"g_"+match[i][0]+","+match[i][1]+";"
        strtype=strtype+"|"
        #上映参数
        match=re.compile('href="/v_olist/c_'+list[1:]+'_a__s__g__r_(.+?)_d_1_fv_0_fl__o_7.html">(.+?)</a>').findall(link)
        for i in range(0,len(match)):
...
def Movie4(name,list,type,kind,area,time,sort,page):
	...
	u=sys.argv[0]+"?mode=40&name="+urllib.quote_plus(name)
	xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li)
       	match=re.compile('<liclass="show_img"><imgsrc="(.+?)"alt="(.+?)"(.+?)href="http://www.youku.com/show_page/id_(.+?).html"(.+?)<spanclass="num">(.+?)</span>(.+?)<spanclass="num">(.+?)</span>').findall(link)
        if len(match)>0:
        	for i in range(0,len(match)):
			li=xbmcgui.ListItem(str(i+1)+"."+match[i][1]+" 【"+match[i][5].replace(',','')+","+match[i][7]+"】",match[i][0],match[i][0])
                        if list=="c96":
                                u=sys.argv[0]+"?mode=16&name="+urllib.quote_plus(match[i][1])+"&url="+urllib.quote_plus(match[i][3])
			        xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li)
                        else:
                                u=sys.argv[0]+"?mode=17&name="+urllib.quote_plus(match[i][1])+"&url="+urllib.quote_plus(match[i][3])+"&upage="+urllib.quote_plus("1")
			        xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li,True)
       	match=re.compile('divclass="LPageBar">(.+?)</div>').findall(link)
        if len(match)>0:
...
def Movie6(name,url,upage):
	...
	xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li)
        ...
        link=re.sub("\t","",link)
        link=re.sub(" ","",link)
      	match=re.compile('<divclass="show_img">(.+?)href="http://v.youku.com/v_show/id_(.+?).html"target="video"><imgsrc="(.+?)"alt="(.+?)"').findall(link)
        if len(match)>0:
        	for i in range(0,len(match)):
			li=xbmcgui.ListItem(str(i+1)+"."+match[i][3],match[i][2],match[i][2])
                        u=sys.argv[0]+"?mode=18&name="+urllib.quote_plus(match[i][3])+"&url="+urllib.quote_plus(match[i][1])+"&director="+urllib.quote_plus("")+"&studio="+urllib.quote_plus("")+"&plot="+urllib.quote_plus("")
			xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li)
       	match=re.compile('<spanclass="num">(.+?)</span>').findall(link)
        if len(match)>0:
                pagenum=int(match[0].split("/")[1])
        	for i in range(0,pagenum):
                        if str(i+1)!=upage:
		                li=xbmcgui.ListItem("..第"+str(i+1)+"页")
		               	u=sys.argv[0]+"?mode=17&name="+urllib.quote_plus(name)+"&url="+urllib.quote_plus(url)+"&upage="+urllib.quote_plus(str(i+1))
		               	xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li,True)

def PlayVideo(name,url,director,studio,plot):
        req = urllib2.Request("http://www.flvcd.com/parse.php?kw=http://v.youku.com/v_show/id_"+url+".html")
       	req.add_header("User-Agent", "Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3")
        response = urllib2.urlopen(req)
        link=response.read()
       	response.close()
