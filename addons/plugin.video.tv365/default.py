          ...
          for td in match:
                ...
                if len(match1)==1:
                        if match1[0][0].find('/itv/')!=-1:
                              num=num+1
                              url='http://www.tv365w.com'+match1[0][0].replace('..','')
		              li=xbmcgui.ListItem(str(num)+'.'+match1[0][1])
		              u=sys.argv[0]+"?mode=2&name="+urllib.quote_plus(match1[0][1])+"&url="+urllib.quote_plus(url)
		              xbmcplugin.addDirectoryItem(int(sys.argv[1]),u,li,True)
